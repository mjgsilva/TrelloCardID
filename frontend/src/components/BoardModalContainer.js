...
export default class BoardModalContainer extends Component {
  ...
  componentDidMount() {
    getMyBoards().then(boards => {
      const sourceBoards = boards.map(board => ({
        value: board.id,
        label: board.name
      }));
  ...
  onSubmit = newCounter => {
    ...
    createNewCounter(newCounter).then(res => {
      addBoardAndClose(newCounter);
    });
  };

  render() {
    const { props: { closeModal }, state: { boards }, onSubmit } = this;

    return (
      <BoardModal boards={boards} onSubmit={onSubmit} closeModal={closeModal} />
    );
  }
}
